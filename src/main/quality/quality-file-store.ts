...
export class QualityFileStore {
  private readonly writableBase = getWritableBasePath();
  private readonly uploadRoot = path.join(this.writableBase, 'uploads', 'quality');
  ...
  async saveFiles(taskId: string, itemId: string, files: QualityAttachmentUpload[]): Promise<QualityAttachmentRecord[]> {
    ...
    for (const file of files) {
      ...
      const relativePath = toRelativeUploadPath(path.relative(this.writableBase, absolutePath));
      saved.push({
        relativePath,
        originalName,
        fileName,
  ...
  decorateAttachment(record: QualityAttachmentRecord): QualityAttachmentView {
    ...
    return {
      ...record,
      isImage,
      fileUrl: isImage ? pathToFileURL(absolutePath).href : undefined,
    };
  }
  ...
  resolveRelativePath(relativePath: string): string {
    ...
    const absolutePath = path.resolve(this.writableBase, normalized);
    const baseWithSep = path.resolve(this.writableBase) + path.sep;
    if (absolutePath !== path.resolve(this.writableBase) && !absolutePath.startsWith(baseWithSep)) {
...
function toRelativeUploadPath(relativePath: string): string {
