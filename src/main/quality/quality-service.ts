...
export class QualityService {
  ...
  async createTask(input: QualityCreateTaskInput): Promise<QualityTaskWithItems & { taskId: string; submitToken: string; submitLink: string }> {
  ...
  async openAttachment(relativePath: string): Promise<void> {
    await this.fileStore.openAttachment(requireText(relativePath, '\u7f3a\u5c11\u9644\u4ef6\u8def\u5f84'));
  }

  buildSubmitLink(task: Pick<QualityTaskRecord, 'id' | 'submitToken'>): string {
    const submitPage = path.join(__dirname, '../renderer/pages/quality/quality-submit.html');
    const url = pathToFileURL(submitPage);
    url.searchParams.set('taskId', task.id);
    url.searchParams.set('submitToken', task.submitToken);
    return url.toString();
  }
