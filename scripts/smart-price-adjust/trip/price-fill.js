...
const { throwTripError, normalizeText, compactText, normalizePrice } = require("./mapper");
...
async function fillRoomPriceWithRetry(page, room) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    if (!match) {
      throwTripError(
        ...
        "fill_price",
        `Trip price row not found: ${room.roomName}`,
    ...
    if (!(await input.count()) || !(await input.isVisible().catch(() => false))) {
      throwTripError(
        "PRICE_INPUT_NOT_FOUND",
        "fill_price",
        `Trip price input not found: ${room.roomName}`,
  ...
  throwTripError(
    ...
    "verify_price_echo",
    `Trip price echo mismatch: ${room.roomName}`,
    {
      roomName: room.roomName,
...
function scoreProductTitle(titleText, strategy) {
  ...
  if (!text.includes("pre-sale") && !text.includes("presale") && !text.includes("\u9884\u552e")) score += 30;
  if (text.includes("no breakfast") || text.includes("<\u65e0\u65e9>")) score += 20;
  return score;
}
