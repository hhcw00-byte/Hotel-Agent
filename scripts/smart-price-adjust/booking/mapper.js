...
function buildBookingSuccess(segment, details = {}) {
  ...
  const submitted = saveResult.submitted !== false;
  const submitClicked = saveResult.submitClicked !== false && submitted;
  return {
    ok: true,
    success: true,
    submitted,
    alreadyApplied: Boolean(saveResult.alreadyApplied),
    noChangeNeeded: Boolean(saveResult.noChangeNeeded),
    summary: {
...
function buildBookingRoomSuccess(room, details = {}) {
  return {
    ...
    ok: true,
    success: true,
    matchedRatePlan: details.matchedRatePlan || "",
