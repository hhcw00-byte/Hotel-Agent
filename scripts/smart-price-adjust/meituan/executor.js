...
async function executeMeituanSegment(context, options = {}) {
  ...
  try {
    ...
    await dismissBlockingModals(page);
    const datePageReady = await prepareMeituanPageForDateSelection(page);
    await selectDateRange(page, {
      startDate: segment.startDate,
      endDate: segment.endDate
    }, {
      pageReady: datePageReady
    });
    report("\u586b\u5199\u4ef7\u683c");
    await dismissBlockingModals(page);
...
async function prepareMeituanPageForDateSelection(page) {
  ...
  const state = await page.evaluate((selectors) => {
    ...
    const beforeDateControl = Array.from(document.querySelectorAll(selectors.dateRangeControl)).find(visible);
    const attempts = [];
    const targets = [selectors.dateRangeControl, selectors.priceTable, selectors.priceRow, selectors.submitButton];
    for (const selector of targets) {
      const node = Array.from(document.querySelectorAll(selector)).find(visible);
      if (!node) continue;
      node.scrollIntoView({ block: "center", inline: "nearest" });
      attempts.push({ selector, found: true });
      break;
    }
    if (!attempts.length) {
      window.scrollTo(0, document.body.scrollHeight || document.documentElement.scrollHeight || 0);
      attempts.push({ selector: "window.scrollTo(bottom)", found: false });
    }
    const afterScrollY = window.scrollY || document.documentElement.scrollTop || document.body.scrollTop || 0;
    const dateControl = Array.from(document.querySelectorAll(selectors.dateRangeControl)).find(visible);
    const box = dateControl ? dateControl.getBoundingClientRect() : null;
    return {
      beforeScrollY,
      afterScrollY,
      scrollAttempts: attempts,
      scrolledToBottom: attempts.some((item) => item.selector === "window.scrollTo(bottom)"),
      dateControlVisibleBeforeScroll: Boolean(beforeDateControl),
      dateControlVisibleAfterScroll: Boolean(dateControl),
      dateControlBox: box ? { x: box.x, y: box.y, width: box.width, height: box.height } : null
    };
...
async function createBrowserSession(runtime) {
  ...
  if (!runtime.userDataDir) {
    throwMeituanError("INPUT_RUNTIME_PATH_NOT_FOUND", "runtime_precheck", "runtime.userDataDir is required when connectMode is not electron-bgtab.");
  }
